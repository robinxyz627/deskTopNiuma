<template>
  <div class="side-panel" :style="panelStyle" @click.stop>
    <!-- 工资面板 -->
    <template v-if="type === 'wage'">
      <div class="panel-header">
        <span class="panel-icon">💰</span>
        <span class="panel-title">工资详情</span>
    ...
    <template v-if="type === 'control'">
      <div class="panel-header">
        <span class="panel-icon">💼</span>
        <span class="panel-title">打卡控制</span>
        <button class="panel-close" @click="$emit('close')">✕</button>
      </div>
...
// 根据菜单索引计算面板位置
// 菜单项位置：从上方开始顺时针，每项间隔 60°
// 窗口中心为 (250, 250)（PANEL_SIZE / 2）
const panelStyle = computed(() => {
  const menuRadius = 85 // 菜单项距离中心的半径
  const panelOffset = 80 // 面板距离菜单项的额外偏移
  const angle = (props.menuIndex * 60 - 90) * (Math.PI / 180)
  
  // 窗口中心（面板展开时窗口为 500x500）
  const centerX = 250
  const centerY = 250
  
  // 面板位置：从中心向外延伸
  const panelX = centerX + Math.cos(angle) * (menuRadius + panelOffset)
  const panelY = centerY + Math.sin(angle) * (menuRadius + panelOffset)
